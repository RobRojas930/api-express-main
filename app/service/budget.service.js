...
class BudgetService {
  async findDB(limit, filter = {}) {
    ...
    for (const i = 0; i < budgetDB.length; i++) {
      let categoryBudget = await CategoryModel.findOne({
        categoryId: budgetDB[i].categoryId,
        userId: budgetDB[i].userId,
      });
      if (!categoryBudget) {
  ...
  async updateDB(id, changes) {
    ...
    let budgetOriginal = { ...budget._doc };
    const {
      title,
      initialAmount,
      currentAmount,
  ...
  async getBudgetData(filter = {}) {
    ...
    for (const budget of budgets) {
      ...
      const transactionsByCategoryId = transactions.filter((t) => {
        ...
        const categoryId =
          t.category[0]?.categoryId?.toString() ||
          t.category?.categoryId?.toString();
        return categoryId === budget.categoryId;
      });

      const totalAmount = transactionsByCategoryId.reduce(
        (acc, curr) => acc + curr.amount,
        0
      );

      budget.currentAmount =
        totalAmount > 0 ? budget.initialAmount - totalAmount : 0;
      budget.percentage = (
        ((budget.initialAmount - budget.currentAmount) / budget.initialAmount) *
        100
      ).toFixed(2);
